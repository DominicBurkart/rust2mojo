    ...
    match cli.command {
        Commands::Compile {
            ...
            if stdout {
                ...
                let output_path = output.unwrap_or_else(|| input.with_extension("mojo"));

                std::fs::write(&output_path, mojo_code).map_err(|e| {
                    rust2mojo::Error::IoError(format!("Failed to write output: {}", e))
                })?;
